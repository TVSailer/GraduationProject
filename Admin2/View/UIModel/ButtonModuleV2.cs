...
public sealed class ButtonLayoutPanel<TEventArgs> : TableLayoutPanel
{
    ...
    public ButtonLayoutPanel<TEventArgs> SetButtons(IButtons<TEventArgs> buttons) => this.With(_ => Initialize(buttons.GetButtons()));

    private void Initialize(List<CustomButton<TEventArgs>> button)
    {
        ...
        for (int i = 0; i < button.Count; i++)
        {
            ...
            if (Controls[^1] is TableLayoutPanel table)
                table.ControlAddIsColumnPercent(button[i].OnClick(_send, _eventArgs));
        }
