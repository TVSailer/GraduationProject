...
public class ManagmentEntityUi<T, TEntity, TFieldSearch, TFieldDetails, TCard, TButtons>(
    ...
    SearchCardPanel<TEntity, TFieldSearch, TFieldDetails, TCard> searchCardPanel) : View<T> where TEntity : Entity, new()
    where TFieldSearch : PropertyChange, IFieldData
    where TFieldDetails : IFieldData<TEntity>
    where TCard : ObjectCard<TEntity>, new()
    where T : IFieldData
    where TButtons : IButtons<ViewButtonClickArgs<T>>, IButtons<CardClickedArgs<TEntity>>
{
