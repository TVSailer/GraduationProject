...
{
    public class FormDataVisitors : Form
    {
        public readonly BaseCreatingElements CreatingElements;

        public FormDataVisitors(VisitorPresent present)
        {
            ...
            listTextBox.ForEach(x => x.TextChanged += (send, e)
                => present.OnSerchData(listTextBox.Select(x => x.Text).ToArray(), ref dataGridView));
            ...
            var table = CreatingElements.CreateTableLayoutPanel(5, 600, 40, 40, 40, 40, 40, 180)
                ...
                //.ControlsAdd(listLabel.SearchByParameter(Attributes.Serch), 0, 1, 2, 1)
