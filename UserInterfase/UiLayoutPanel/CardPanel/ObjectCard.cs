...
public abstract class ObjectCard<T> : Panel
{
    ...
    public virtual ObjectCard<T> Initialize(object send, T obj)
    {
        Entity = obj;
    ...
    public ObjectCard<T> OnContextMenu(IButtons<CardClickedToolStripArgs<T>>? buttonsContextMenu)
    {
        ...
        var buttons = buttonsContextMenu.GetButtons(new CardClickedToolStripArgs<T>(Entity));
    ...
    private void AddToolStrip(Button button)
    {
        var toolStrip = new ToolStripMenuItem(button.Name);
        toolStrip.Click += (_, _) => button.PerformClick();
        toolStrip.Enabled = button.Enabled;
